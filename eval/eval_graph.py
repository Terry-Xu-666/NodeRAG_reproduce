from Graphrag.Graghrag import init_local_search
import argparse
import pandas as pd
...
async def Response(search_engine,questions,path):
    for index, row in tqdm(questions.iterrows(),total=len(questions)):
        question = row["question"]
        start_time = time.time()
        results = await search_engine.asearch(question)
        end_time = time.time()
        questions.at[index,"response"] = results.response
        questions.at[index,"retrieval"] = results.context_text
        questions.at[index,"time"] = end_time - start_time
        questions.at[index,"tokens"] = count_tokens(results.context_text)
    questions.to_parquet(path.replace('.parquet', 'graph_responsed.parquet'))
    return questions
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f","--output_folder", type=str, help="The output folder of documents")
    ...
    search_engine = init_local_search(args.output_folder)
    questions = asyncio.run(Response(search_engine,questions,args.questions))
    if args.answer:
