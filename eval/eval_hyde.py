...
def Response(search_engine,questions,path):
    for index, row in tqdm(questions.iterrows(),total=len(questions)):
        ...
        response,retrieval = search_engine.answer(question,retrieval=True)
        end_time = time.time()
        questions.at[index,"response"] = response
        questions.at[index,"retrieval"] = retrieval
        questions.at[index,"time"] = end_time - start_time
        questions.at[index,"tokens"] = count_tokens(retrieval)
    questions.to_parquet(path.replace('.parquet', 'hyde_responsed.parquet'))
    return questions
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="Hconfig.yaml", type=str, help="the config file")
    ...
    if args.main_folder:
        config["config"]["main_folder"] = args.main_folder
        
    questions = pd.read_parquet(args.questions)
    
    search_engine = HyDE(HyDEConfig(config),Promptor(HyDEConfig(config)))
