...
def Response(search_engine,questions,path):
    for index, row in tqdm(questions.iterrows(),total=len(questions)):
        ...
        response,retrieval = search_engine.query(question, param=QueryParam(mode="hybrid",with_retrieval_context=True))
        end_time = time.time()
    ...
    questions.to_parquet(path.replace('.parquet', 'light_responsed.parquet'))
    return questions
